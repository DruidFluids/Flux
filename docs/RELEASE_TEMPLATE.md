...
Existing users: open Settings → Updates → Check now to update in place. Or download fluxid-setup-vX.Y.Z.exe below and run it. To verify it first, run Get-FileHash on the file and compare against:
...
Changelog format — keep every release consistent with this template:
  ...
  * Title line is always `# fluxid vX.Y.Z`.
