...
const HELP_SECTIONS: [(&str, &str); 19] = [
    ...
    ("Reorder and Layout", "Drag a tile row in Settings then Tiles to reorder them and the widget reflows live to match. Switch between Vertical and Horizontal layout in Settings then Behavior."),
    ("Tiles Tab", "Each tile has its own row; toggle it on or off, drag to reorder, and expand it with the chevron to pick exactly which details that tile displays (model name, temperature, clock, VRAM, and so on). The Display section at the bottom sets the temperature unit (°C / °F), whether the Network and Disk tiles read in bytes (KB/s) or bits (Mbps, like your router), and the spacing between tiles."),
    ("Behavior", "The Behavior tab is how the widget acts on your desktop: Layout (vertical or horizontal), Always on top, Start minimized to tray, Run at Windows startup, Snap to edges and Snap to windows (with an adjustable snap distance), Click-through, Lock position (so it can't be dragged out of place), a Reset widget position button to bring it back on-screen, and the Update interval that sets how often the stats refresh."),
    ...
    ("Size", "In the Appearance tab: UI scale resizes the whole widget at once; Tile width and Tile height size the individual tiles; Round widget corners toggles the rounded frame; and Opacity sets how see-through the widget is."),
    ...
    ("Game Mode", "Bind a hotkey to instantly snap a compact overlay into a corner of your primary monitor, even in fullscreen. Pick the corner, opacity, orientation, optional click-through, and which tiles show while it is active. A small badge on the overlay shows the exit hotkey. Press the hotkey again to send it back."),
