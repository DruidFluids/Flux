...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=FLUXID_PAYLOAD");

    match env::var_os("FLUXID_PAYLOAD") {
        Some(src) if !src.is_empty() => {
            ...
            match fs::copy(&src, &dest) {
                ...
                Err(e) => panic!(
                    "FLUXID_PAYLOAD is set to {} but it could not be read: {e}",
                    src.display()
                ),
            }
        }
        _ => {
            ...
            println!(
                "cargo:warning=FLUXID_PAYLOAD not set — building installer with an EMPTY payload (dev build, cannot install)."
            );
        }
    }
}
