...
//!
//! Faithful port of the C# `CpuSensorDriver`. fluidMonitor **never bundles or
...
fn temp_installer_path() -> PathBuf {
    let stamp = std::time::SystemTime::now()
        ...
        .map(|d| d.as_nanos())
        .unwrap_or(0);
    std::env::temp_dir().join(format!("fluidMonitor_sensor_{stamp:x}.exe"))
}

async fn download_installer() -> Result<Vec<u8>, String> {
    let client = reqwest::Client::builder()
        .user_agent("fluidMonitor")
